...
#define U8TO32_LITTLE(c) \
    (((u32)T8(*(c))) | ((u32)T8(*((c) + 1)) << 8) | \
    (u32)T8(*((c) + 2)) << 16) | ((u32)T8(*((c) + 3)) << 24))

#define U32TO8_BIG(c, v) \
    do { u32 x = (v); u8 *d = (c); d[0] = T8(x >> 24); d[1] = T8(x >> 16); \
    d[2] = T8(x >> 8); d[3] = T8(x); } while (0)

#define U32TO8_LITTLE(c, v) \
    do { u32 x = (v); u8 *d = (c); d[0] = T8(x); d[1] = T8(x >> 8); \
    d[2] = T8(x >> 16); d[3] = T8(x >> 24); } while (0)
...
typedef struct whirlpool_ctx_s
{
	std::uint8_t  bit_length[LENGTHBYTES];
	std::uint8_t  buffer[WBLOCKBYTES];
	std::int32_t buffer_bits;
	std::int32_t buffer_pos;
	std::uint64_t hash[DIGESTBYTES/8];
} whirlpool_ctx_t;

#define R 10

static const std::uint64_t C0[256] =
{
...
static const std::uint64_t C1[256] =
{
...
static const std::uint64_t C2[256] =
{
...
static const std::uint64_t C3[256] =
{
...
static const std::uint64_t C4[256] =
{
...
static const std::uint64_t C5[256] =
{
...
static const std::uint64_t C6[256] =
{
...
static const std::uint64_t C7[256] =
{
...
static const std::uint64_t rc[R + 1] =
{
...
static void process_buffer(whirlpool_ctx_t * const ctx)
{
    std::uint64_t K[8];
    std::uint64_t block[8];
    std::uint64_t state[8];
    std::uint64_t L[8];
    std::uint8_t * buffer = ctx->buffer;

    for (auto i = 0; i < 8; i++, buffer += 8)
    {
        block[i] =
            (((std::uint64_t)buffer[0]) << 56) ^
            (((std::uint64_t)buffer[1] & 0xffL) << 48) ^
            (((std::uint64_t)buffer[2] & 0xffL) << 40) ^
            (((std::uint64_t)buffer[3] & 0xffL) << 32) ^
            (((std::uint64_t)buffer[4] & 0xffL) << 24) ^
            (((std::uint64_t)buffer[5] & 0xffL) << 16) ^
            (((std::uint64_t)buffer[6] & 0xffL) <<  8) ^
            (((std::uint64_t)buffer[7] & 0xffL))
        ;
    }
...
void whirlpool_init(whirlpool_ctx_t * const ctx)
{
    std::memset(ctx->bit_length, 0, 32);
    
    ctx->buffer_bits = ctx->buffer_pos = 0;
...
void whirlpool_add(
    const std::uint8_t * const source, const std::size_t & len,
    whirlpool_ctx_t * const ctx
    )
{
    auto source_bits = len * 8;

    std::int32_t source_pos = 0;
    std::int32_t source_gap = (8 - ((int)source_bits & 7)) & 7;
    std::int32_t buffer_rem = ctx->buffer_bits & 7;
    std::int32_t i;
    std::uint32_t b, carry;
    std::uint8_t * buffer = ctx->buffer;
    std::uint8_t * bit_length = ctx->bit_length;
    std::int32_t buffer_bits = ctx->buffer_bits;
    std::int32_t buffer_pos = ctx->buffer_pos;

    std::uint64_t value = source_bits;
    
    for (i = 31, carry = 0; i >= 0 && (carry != 0 || value != LL(0)); i--)
    {
        carry += bit_length[i] + ((std::uint32_t)value & 0xff);
        bit_length[i] = (std::uint8_t)carry;
    ...
    while (source_bits > 8)
    {
        ...
        buffer[buffer_pos++] |= (std::uint8_t)(b >> buffer_rem);
        buffer_bits += 8 - buffer_rem;
        
        if (buffer_bits == DIGESTBITS)
        {
            process_buffer(ctx);
        ...
        buffer[buffer_pos] = b << (8 - buffer_rem);
    ...
    if (source_bits > 0)
    {
        ...
        buffer[buffer_pos] |= b >> buffer_rem;
    }
    else
    {
        b = 0;
    ...
    ctx->buffer_bits = buffer_bits;
    ctx->buffer_pos = buffer_pos;
}

void whirlpool_final(
    whirlpool_ctx_t * const ctx, std::uint8_t * const result
    )
{
    std::uint8_t * buffer = ctx->buffer;
    std::uint8_t * bit_length = ctx->bit_length;
    int buffer_bits  = ctx->buffer_bits;
    int buffer_pos   = ctx->buffer_pos;
    std::uint8_t * digest = result;
    ...
    if (buffer_pos > WBLOCKBYTES - LENGTHBYTES)
    {
        ...
        {
            std::memset(&buffer[buffer_pos], 0, WBLOCKBYTES - buffer_pos);
        }
    ...
    {
        std::memset(
            &buffer[buffer_pos], 0, (WBLOCKBYTES - LENGTHBYTES) - buffer_pos
        );
    }
    
    buffer_pos = WBLOCKBYTES - LENGTHBYTES;

    std::memcpy(&buffer[WBLOCKBYTES - LENGTHBYTES], bit_length, LENGTHBYTES);

    process_buffer(ctx);

    for (auto i = 0; i < DIGESTBYTES / 8; i++)
    {
        digest[0] = (std::uint8_t)(ctx->hash[i] >> 56);
        digest[1] = (std::uint8_t)(ctx->hash[i] >> 48);
        digest[2] = (std::uint8_t)(ctx->hash[i] >> 40);
        digest[3] = (std::uint8_t)(ctx->hash[i] >> 32);
        digest[4] = (std::uint8_t)(ctx->hash[i] >> 24);
        digest[5] = (std::uint8_t)(ctx->hash[i] >> 16);
        digest[6] = (std::uint8_t)(ctx->hash[i] >>  8);
        digest[7] = (std::uint8_t)(ctx->hash[i]);
        digest += 8;
    }
...
void print(const std::uint8_t * buf, const std::size_t & len)
{
    ...
    )
{
    ...
    std::uint8_t xored[32];
    
	for (auto i = 0; i < sizeof(xored); i++)
	{
        xored[i] =
	...
	std::memcpy(digest, xored, 32);
}

bool whirlpool_test()
{
    std::uint32_t piece_len, total_len, data_len;
    
    whirlpool_ctx_t w;

    std::uint8_t
        data_buf[512], expected_digest[DIGESTBYTES],
        computed_digest[DIGESTBYTES]
    ;
    ...
    for (data_len = 0; data_len <= sizeof(data_buf); data_len++)
    {
        ...
        whirlpool_init(&w);
        whirlpool_add(data_buf, data_len, &w);
        whirlpool_final(&w, expected_digest);
        ...
        if (data_len > 0)
        {
            for (piece_len = 1; piece_len <= data_len; piece_len++)
            {
                ...
                for (
                    total_len = 0; total_len + piece_len <= data_len;
                    total_len += piece_len
                    )
                {
                    whirlpool_add(data_buf + total_len, piece_len, &w);
                }
                
                if (total_len < data_len)
                {
                    whirlpool_add(
                        data_buf + total_len, (data_len - total_len), &w
                    );
                }
                
                whirlpool_final(&w, computed_digest);
                
                if (
                    std::memcmp(computed_digest, expected_digest,
                    DIGESTBYTES) != 0
                    )
                {
                    fprintf(stderr, "API error @ piece_len = %d\n", piece_len);
                    
                    print(computed_digest, DIGESTBYTES); printf("\n\n");
                    
                    print(expected_digest, DIGESTBYTES); printf("\n\n");
        ...
        else
        {
            ...
            if (
                std::memcmp(computed_digest, expected_digest, DIGESTBYTES) != 0
                )
            {
